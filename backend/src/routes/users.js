...
router.post('/', authorize('head_pastor','pastor'), [
  ...
  body('role').notEmpty().isIn(['head_pastor', 'pastor', 'director', 'hod', 'branch_pastor', 'branch_admin', 'member']),
], handleValidationErrors, c.inviteUser);
router.put('/:id', authorize('head_pastor','pastor'), [
  body('email').optional().isEmail().normalizeEmail(),
  body('firstName').optional().trim().escape(),
  body('lastName').optional().trim().escape(),
  body('role').optional().isIn(['head_pastor', 'pastor', 'director', 'hod', 'branch_pastor', 'branch_admin', 'member']),
